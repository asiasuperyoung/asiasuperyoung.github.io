...
function populateTableEntry(trainee) {
  ...
  <div class="table__entry ${eliminated}">
    <div class="table__entry-icon">
      ...
      <div class="table__entry-icon-border ${trainee.eval}-rank-border"></div>
      ${
    ...
    <div class="table__entry-text">
      ...
      <span class="affiliationandyear">${trainee.affiliation.toUpperCase()} •
...
function populateRankingEntry(trainee, currRank) {
  let modifiedaffiliation = trainee.affiliation.toUpperCase();
  modifiedaffiliation = modifiedaffiliation.replace("ENTERTAINMENT", "ENT.");
  if (abbreviatedCompanies[modifiedaffiliation]) {
    modifiedaffiliation = abbreviatedCompanies[modifiedaffiliation];
  }
  let eliminated = (showEliminated && trainee.eliminated) && "eliminated";
  let top7 = (showTop7 && trainee.top7) && "top7";
  const rankingEntry = `
  <div class="ranking__entry ${eliminated}">
    ...
    <div class="ranking__row-text">
      ...
      <div class="affiliation">${modifiedaffiliation}</div>
    </div>
  </div>`;
  return rankingEntry;
}
...
function filterTrainees(event) {
  ...
  // filters trainees based on name, alternate names, and affiliation
  filteredTrainees = trainees.filter(function (trainee) {
    let initialMatch = includesIgnCase(trainee.name_romanized, filterText) || includesIgnCase(trainee.affiliation, filterText);
...
const currentURL = "https://asiasuperyoung.github.io/";
